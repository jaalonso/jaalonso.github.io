...
% óptima(?S) se verifica si S es una solución del problema mediante
% búsqueda óptima; es decir, S es la solución obtenida por búsqueda
% óptima a partir de [0-[E]], donde E el estado inicial. 
óptima(S) :-
   estado_inicial(E),
   óptima([0-[E]],S).

% óptima(+Abiertos,?S) se verifica si S es una solución del problema a
% partir de la lista de nodos Abiertos encontrada por búsqueda
