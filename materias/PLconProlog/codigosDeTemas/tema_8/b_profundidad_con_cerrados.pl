% b_profundidad_con_cerrados.pl
% Búsqueda en profundidad con cerrados.
% José A. Alonso Jiménez <https://jaalonso.github.io>
% Sevilla, 25-mayo-2022
% ======================================================================

:- module(b_profundidad_con_cerrados, [profundidad_con_cerrados/1]).
...
% profundidad_con_cerrados(?S) se verifica si S es una solución del
% problema mediante búsqueda en profundidad. El procedimiento es
% 1. Sea E el estado inicial.
% 2. La solución S es la obtenida por búsqueda en profundidad con [[E]] como la
%    lista de abiertos y [] como la lista de cerrados.
profundidad_con_cerrados(S) :-
   estado_inicial(E),                              % 1
   profundidad_con_cerrados([[E]],[ ],S).          % 2

% profundidad_con_cerrados(+Abiertos,+Cerrados,?Solucion) se verifica si
...
%         dichos Sucesores al principio de R y
%    2.4. los nuevos cerrados, NCerrados, es la lista obtenida añadiendo E a
%         Cerrados,
%    entonces
%    2.5. S es la solución obtenida por búsqueda en profundidad con los nuevos
%         abiertos y los nuevos cerrados.
profundidad_con_cerrados(Abiertos,_,S) :-
   Abiertos = [[E|C]|_],                            % 1.1
   estado_final(E),                                 % 1.2
   reverse([E|C],S).                                % 1.3
profundidad_con_cerrados(Abiertos,Cerrados,S) :-
   Abiertos = [[E|_]|R],                            % 2.1
   expande(Abiertos,Cerrados,Sucesores),            % 2.2
   append(Sucesores,R,NAbiertos),                   % 2.3
   NCerrados = [E|Cerrados],                        % 2.4
   profundidad_con_cerrados(NAbiertos,NCerrados,S). % 2.5
