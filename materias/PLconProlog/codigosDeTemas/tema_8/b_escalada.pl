...
%   solución obtenida por búsqueda en escalada a partir de 0-[E].
escalada(Sol) :-
   estado_inicial(E),
   heuristica(E,H),
   escalada(H-[E],Sol).
...
% 2. Si
%         N=H-[E|C] y
%     2.1 E1 es un sucesor de E con heurística H1 < H y E no tiene sucesores
%         con heurística menor que H1
%    entonces
%     2.2 S es la solución por por escalada a partir de H1-[E1,E|R].
escalada(_H-[E|R],S) :-
   estado_final(E),                        % 1.1
   reverse([E|R],S).                       % 1.2
escalada(H-[E|R],S) :-
   mejor_sucesor(E,H,E1,H1),
   escalada(H1-[E1,E|R],S).
